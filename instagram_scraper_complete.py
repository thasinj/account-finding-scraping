#!/usr/bin/env python3
"""
Instagram Hashtag Profile Scraper - COMPLETE VERSION
Uses pagination + profile data fetching for comprehensive results
"""
...
class InstagramScraper:
    ...
    def search_hashtag_with_pagination(self, hashtag: str, max_pages: int = 5) -> List[Dict]:
        """Search hashtag with pagination to get more comprehensive results"""
    ...
    def get_profile_details(self, username: str) -> Optional[Dict]:
        ...
        url = f"{self.base_url}/user_info.php?username={username}"
    ...
    def enrich_profiles_with_details(self, users: List[Dict], max_workers: int = 5) -> List[Dict]:
            ...
            if profile_details:
                ...
                enriched_profile = {
                    ...
                    **profile_details,  # Detailed profile data
                    'data_source': 'hashtag_search + profile_api'
                }
                print(f"  ✅ @{username}: {self._format_number(profile_details.get('followers', 0))} followers")
                return enriched_profile
            else:
                print(f"  ⚠️  @{username}: Could not fetch profile details")
        ...
        print(f"\n✅ Profile enrichment complete! {len(enriched_profiles)} profiles with data")
    ...
    def print_results(self, profiles: List[Dict], hashtag: str):
        ...
        print(f"\n🎯 Found {len(profiles)} profiles for #{hashtag} (with follower data)")
        print("=" * 100)
    ...
    def save_to_json(self, profiles: List[Dict], hashtag: str):
        ...
        filename = f"instagram_profiles_complete_{hashtag}_{timestamp}.json"
        ...
        data = {
            ...
            'api_endpoints_used': [
                '/search_hashtag.php (with pagination)',
                '/user_info.php (for profile details)'
            ],
            'data_includes': [
                'usernames', 'follower_counts', 'following_counts',
                'post_counts', 'verification_status', 'bio_information',
                'profile_urls', 'sample_posts'
            ],
            'profiles': profiles
        }
...
def main():
    ...
    print("🚀 Instagram Complete Profile Scraper")
    ...
    # Step 1: Search hashtag with pagination
    users = scraper.search_hashtag_with_pagination(hashtag, max_pages)
    
    if users:
        # Step 2: Enrich with detailed profile data
        ...
        if enriched_profiles:
            ...
            print(f"📊 Summary: {len(enriched_profiles)} profiles with full data for #{hashtag}")
            print(f"🎯 Top profile: @{enriched_profiles[0].get('username')} with {scraper._format_number(enriched_profiles[0].get('followers', 0))} followers")
        else:
